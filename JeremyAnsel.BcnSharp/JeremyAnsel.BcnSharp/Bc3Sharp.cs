...
namespace JeremyAnsel.BcnSharp
{
    public static class Bc3Sharp
    {
        ...
        {
            NativeMethods.BC3_Encode(pBlock, pPixelsRGBA, width, height);
        }
        ...
        public static void Encode(byte[] pBlock, byte[] pPixelsRGBA, int width, int height)
        {
            ...
            try
            {
                NativeMethods.BC3_Encode(handleCompressed.AddrOfPinnedObject(), handle.AddrOfPinnedObject(), width, height);
            }
            finally
            {
                handle.Free();
        ...
        {
            NativeMethods.BC3_Decode(pBlock, pPixelsRGBA, width, height);
        }
        ...
        public static void Decode(byte[] pBlock, byte[] pPixelsRGBA, int width, int height)
        {
            ...
            try
            {
                NativeMethods.BC3_Decode(handleCompressed.AddrOfPinnedObject(), handle.AddrOfPinnedObject(), width, height);
            }
            finally
            {
                handle.Free();
