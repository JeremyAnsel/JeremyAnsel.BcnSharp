...
namespace JeremyAnsel.BcnSharp
{
    public static class Bc5Sharp
    {
        ...
        {
            NativeMethods.BC5_Encode(pBlock, pPixelsRGBA, width, height);
        }
        ...
        public static void Encode(byte[] pBlock, byte[] pPixelsRGBA, int width, int height)
        {
            ...
            try
            {
                NativeMethods.BC5_Encode(handleCompressed.AddrOfPinnedObject(), handle.AddrOfPinnedObject(), width, height);
            }
            finally
            {
                handle.Free();
        ...
        {
            NativeMethods.BC5_Decode(pBlock, pPixelsRGBA, width, height);
        }
        ...
        public static void Decode(byte[] pBlock, byte[] pPixelsRGBA, int width, int height)
        {
            ...
            try
            {
                NativeMethods.BC5_Decode(handleCompressed.AddrOfPinnedObject(), handle.AddrOfPinnedObject(), width, height);
            }
            finally
            {
                handle.Free();
